...
$year = $db->query('SELECT DISTINCT year FROM history_support;');
$results = $db->query('SELECT * FROM okr_support;');
...
if ($selectYear == "all") {
    if ($selectTopics == "all") {
        $historyResults = $db->prepare('SELECT success, count(*) as count FROM history_support GROUP by success');
    } else {
        $historyResults = $db->prepare('SELECT success, count(*) as count FROM history_support WHERE topics like :selectTopics GROUP by success');
    }
    ...
    if ($selectTopics == "all") {
        $historyResults = $db->prepare("SELECT success, count(*) as count FROM history_support WHERE year=='$selectYear' GROUP by success");
    } else {
        $historyResults = $db->prepare('SELECT success, count(*) as count FROM history_support WHERE topics like :selectTopics AND year = :selectYear GROUP by success;');
    }
    $history = $historyResults->execute();
    while ($row = $history->fetchArray()) {
        array_push($data, $row);
                ...
                <div class="card m-3">
                    ...
                    <div class="card-body">
                        <div class="row d-flex justify-content-evenly">
                                    ...
                                    <div class="card-body text-center">
                                        <blockquote class="blockquote mb-0">
                                            <p class="fw-bolder" style="color: #953904">
                                                ...
                                                <a href="./analyze.php" class="stretched-link">สายผู้สอน</a>
                                            </p>
                                        </blockquote>
                                    </div>
                            ...
                            <div class="col-3 p-4">
                                <div class="card">
                                    <div class="card-body text-center" style="background-color: #d7f3d7">
                                        <blockquote class="blockquote mb-0">
                                            <p class="fw-bolder" style="color: #953904">
                                                <i class="fas fa-users"></i> <br />
                                                <a href="#" class="stretched-link">สายสนับสนุน</a>
                                            </p>
                                        </blockquote>
                                    </div>
